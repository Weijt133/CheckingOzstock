...
def checkDate(d):
    today = date.today()
    print("One item is expired " + str(today - d))
...
def updateStoreStatus():
    if os.path.exists('stockstatus.xlsx'):
        ...
        for rowNumber in range(2, ws.max_row + 1):
            if (checkOzstock(ws["B" + str(rowNumber)].value) == "Out of stock" or checkEbay(
                    ws["C" + str(rowNumber)].value) == "Out of stock"):
                ...
                if (checkDate(d)):
                    ws["F" + str(rowNumber)].fill = PatternFill(start_color='FFFF0000',
                                                                end_color='FFFF0000',
                                                                fill_type='solid')
                    ws["E" + str(rowNumber)].fill = PatternFill(start_color='FFFF0000',
                                                                end_color='FFFF0000',
                                                                fill_type='solid')
